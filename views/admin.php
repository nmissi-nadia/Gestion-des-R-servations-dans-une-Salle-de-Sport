...
if (isset($_GET['supprimer']) && isset($_GET['id_Membre'])) {
    ...
    try {
        ...
        $stmt->bindParam(':id_Membre', $idMembre, PDO::PARAM_INT);
        $stmt->execute();
...
<div class="container mx-auto mt-8 p-4 bg-purple-100 shadow-md rounded justify-self-center  w-3/5">
        ...
        <form method="POST" action="add_activite.php" class="space-y-4">
            ...
            <div>
                ...
                <textarea id="description" name="description" required class="mt-1 p-2 border rounded w-full"></textarea>
            </div>
            
            <div>
                <label for="capacite" class="block font-medium text-gray-700">Capacité</label>
                <input type="number" id="capacite" name="capacite" required class="mt-1 p-2 border rounded w-full">
            </div>
            
            <div>
                <label for="date_debut" class="block font-medium text-gray-700">Date de début</label>
                <input type="date" id="date_debut" name="date_debut" required class="mt-1 p-2 border rounded w-full">
            </div>
